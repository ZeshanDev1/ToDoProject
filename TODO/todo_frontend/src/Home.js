...
const Home = () => {
    ...
    useEffect(() => {
        axios.get('http://54.152.170.109:5000/get')
            .then(result => setTodos(result.data))
            .catch(err => console.log(err));
    }, []);

    const edit = (id) => {
        axios.put(`http://54.152.170.109:5000/edit/${id}`)
            .then(result => {
    ...
    const Update = (id, updatedTask) => {
        axios.put(`http://54.152.170.109:5000/update/${id}`, { task: updatedTask })
            .then(result => {
                console.log(result.data);
                const updatedTodos = todos.map(todo => {
                    if (todo._id === id) {
    ...
    const Hdelete = (id) => {
        axios.delete(`http://54.152.170.109:5000/delete/${id}`)
            .then(result => {
                console.log(result.data);
